 ...
 */
public class DijkstraWithTCWithHeuristic {
    ...
    Heuristic<BaseVertex> heuristic;

    public DijkstraWithTCWithHeuristic(GridDirectedGraph g, double mtr)
    {
        graph = g;
        minTurnRadius = mtr;
        initMap();
    ...
    public DijkstraWithTCWithHeuristic(GridDirectedGraph g, double mtr, double maxTurnAngle)
    {
        graph = g;
        minTurnRadius = mtr;
    ...
    public DijkstraWithTCWithHeuristic(GridDirectedGraph g, double mtr, double maxTurnAngle, MHDHeuristic<BaseVertex> heuristic)
    {
        graph = g;
        minTurnRadius = mtr;
        this.maxTurnAngle = maxTurnAngle;
    ...
    private void initMap() {
        ...
        for (BaseVertex v: graph.vertexSet())
        {
            ...
            /*MODIFIED*/
            double distanceWithHeuristic = entry.getDistance()+ ((heuristic!=null) ? heuristic.getValue(gv):0 );
            heap.insert(entry.getHeapNode(),distanceWithHeuristic);
        }
    ...
    public GraphPath<BaseVertex,BaseEdge> getShortestPath()
    {
        while(!heap.isEmpty())
        {
            ...
            for (BaseVertex v:currentVertex.getOutgoingNeighbors())
            {
                ...
                if ( ! constraintsSatisfied(currentVertexEntry,currentNode,nextVertex,tempNode))
                ...
                if (dist < nextVertexEntry.getDistance())
                {
                    ...
                    /*MODIFIED*/
                    heap.decreaseKey(nextVertexEntry.getHeapNode(),dist+((heuristic!=null) ? heuristic.getValue(nextVertex):0 ));
                }
            }
        }
